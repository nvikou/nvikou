<div align="center">

# Hi, I'm Vikou

**Full-Stack Developer, Data Scientist & AI Builder**

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=500&size=22&duration=3800&pause=900&color=00D4AA&center=true&vCenter=true&width=640&lines=I+build+AI+agents+%26+full-stack+apps;PhD+in+Statistics+%26+Econometrics;Turning+data+into+real-world+impact" alt="Typing animation" />

[![Portfolio](https://img.shields.io/badge/Portfolio-nvikou.github.io-00d4aa?style=for-the-badge&logo=google-chrome&logoColor=white)](https://nvikou.github.io)
[![Email](https://img.shields.io/badge/Email-nvikou%40gmail.com-ea4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:nvikou@gmail.com)
[![Kaggle](https://img.shields.io/badge/Kaggle-nelsonvikou-20BEFF?style=for-the-badge&logo=kaggle&logoColor=white)](https://www.kaggle.com/nelsonvikou)

</div>

---

## About me

Passionate about technology, I use **data and AI/ML** to solve business problems — building **AI-powered tools**, **conversational agents**, and **scalable web applications**.

PhD in **Statistics & Econometrics**. I work across **AI/ML development**, **GPT engineering**, and full-stack delivery — and I ship production-ready systems, not just demos:

- Self-hosted LLM infrastructure (Open WebUI, LiteLLM, SSO, admin ops)
- Customer support automation (RAG, confidence scores, human escalation)
- AI chatbots & Telegram bots with memory and personality
- Advanced statistical modeling (Tobit regression, censored data)
- Reproducible AutoML pipelines & data science notebooks

> *Research rigor. Engineering delivery.*

---

## Tech stack

<div align="center">

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=flat-square&logo=fastapi&logoColor=white)
![React](https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=react&logoColor=black)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white)
![OpenAI](https://img.shields.io/badge/OpenAI-412991?style=flat-square&logo=openai&logoColor=white)
![LiteLLM](https://img.shields.io/badge/LiteLLM-0A0A0A?style=flat-square&logo=openai&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=flat-square&logo=postgresql&logoColor=white)
![LangChain](https://img.shields.io/badge/LangChain-1C3C3C?style=flat-square&logo=langchain&logoColor=white)
![scikit-learn](https://img.shields.io/badge/scikit--learn-F7931E?style=flat-square&logo=scikit-learn&logoColor=white)
![Jupyter](https://img.shields.io/badge/Jupyter-F37626?style=flat-square&logo=jupyter&logoColor=white)

</div>

| Area | Technologies |
|:-----|:---------------|
| **Frontend** | React, TypeScript, Vite, Tailwind CSS, HTML/CSS |
| **Backend** | Python, FastAPI, Node.js, WebSocket, Pydantic |
| **AI / ML** | OpenAI GPT-4, LangChain, LiteLLM, Open WebUI, ChromaDB, TPOT, scikit-learn |
| **Data** | Pandas, NumPy, SciPy, Jupyter, Kaggle |
| **DevOps** | Docker, Docker Compose, Caddy, Authentik, Git, Dev Containers |

---

## Featured projects

<table>
<tr>
<td width="50%" valign="top">

### AetherGate — LLM Stack
Self-hosted AI gateway: Open WebUI + LiteLLM + admin console, SSO, Postgres — local & cloud LLMs in ~60 seconds.

`Docker` · `LiteLLM` · `Open WebUI` · `FastAPI` · ⭐ 8

[![Repo](https://img.shields.io/badge/→_GitHub-1a1a2e?style=flat-square&logo=github)](https://github.com/nvikou/open-webui-litellm-stack)

</td>
<td width="50%" valign="top">

### VateCon — AI Support Agent
Automates **70–80%** of support tickets: RAG, source citations, confidence scores, auto-escalation.

`FastAPI` · `React` · `LangChain` · `ChromaDB` · `Docker` · ⭐ 7

[![Repo](https://img.shields.io/badge/→_GitHub-1a1a2e?style=flat-square&logo=github)](https://github.com/nvikou/Ai-support-chat-react-fastapi)

</td>
</tr>
<tr>
<td valign="top">

### NeuroHelper
Day plans, checklists & ideas in seconds. React + FastAPI + SSE streaming.

`React 18` · `FastAPI` · `OpenAI` · `Docker`

[![Repo](https://img.shields.io/badge/→_GitHub-1a1a2e?style=flat-square&logo=github)](https://github.com/nvikou/neurohelper-ai)

</td>
<td valign="top">

### AI Psychologist Telegram Bot
**Dr. Émile** — conversational assistant with per-user memory & clinical persona.

`Python` · `aiogram 3` · `GPT-4o-mini` · ⭐ 9

[![Repo](https://img.shields.io/badge/→_GitHub-1a1a2e?style=flat-square&logo=github)](https://github.com/nvikou/Ai-psychologue-bot)

</td>
</tr>
<tr>
<td valign="top">

### Tobit Regression Model
Econometric model for censored data — MLE, scikit-learn-compatible API.

`Python` · `SciPy` · `Econometrics` · ⭐ 8

[![Repo](https://img.shields.io/badge/→_GitHub-1a1a2e?style=flat-square&logo=github)](https://github.com/nvikou/Econometric-Tobit-Regression-Model)

</td>
<td valign="top">

### AutoML with TPOT
Automatic pipeline search, Python export, DevContainer / Codespaces.

`TPOT` · `Jupyter` · `AutoML` · ⭐ 8

[![Repo](https://img.shields.io/badge/→_GitHub-1a1a2e?style=flat-square&logo=github)](https://github.com/nvikou/Automated-Machine-Learning-TPOT)

</td>
</tr>
<tr>
<td valign="top">

### ML Notebooks Collection
Hands-on ML experiments, practical projects, and learning notebooks.

`Jupyter` · `ML` · ⭐ 8

[![Notebooks](https://img.shields.io/badge/→_Notebooks-1a1a2e?style=flat-square&logo=github)](https://github.com/nvikou/Machine-learning-notebooks-collection)

</td>
<td valign="top">

### Math for Data Science
Structured notebooks and resources covering the math foundations of ML.

`Jupyter` · `Math` · ⭐ 8

[![Repo](https://img.shields.io/badge/→_GitHub-1a1a2e?style=flat-square&logo=github)](https://github.com/nvikou/Mathematics-for-Data-Science)

</td>
</tr>
</table>

---

## GitHub snapshot

<div align="center">

[![Followers](https://img.shields.io/github/followers/nvikou?style=for-the-badge&logo=github&label=Followers&color=181717)](https://github.com/nvikou?tab=followers)
[![Repositories](https://img.shields.io/badge/Public%20Repos-8+-00d4aa?style=for-the-badge&logo=github&labelColor=06080f)](https://github.com/nvikou?tab=repositories)
[![Stars](https://img.shields.io/badge/Project%20Stars-56+-6366f1?style=for-the-badge&logo=github&labelColor=06080f)](https://github.com/nvikou?tab=stars)

</div>

| | |
|:--|:--|
| **Main languages** | Python · TypeScript · Jupyter Notebook |
| **Top repos** | [Ai-psychologue-bot](https://github.com/nvikou/Ai-psychologue-bot) · [AetherGate](https://github.com/nvikou/open-webui-litellm-stack) · [VateCon](https://github.com/nvikou/Ai-support-chat-react-fastapi) |
| **Activity** | [Contribution graph](https://github.com/nvikou) |

---

## Goals

- Build **impactful AI tools** that solve real problems
- **Contribute to open source** and share practical knowledge
- Grow a strong **developer & AI community** around meaningful projects

---

## Connect

<div align="center">

**Have a project in mind?** Reach out — I'll reply with a concrete first step.

[![Portfolio](https://img.shields.io/badge/🌐_Portfolio-nvikou.github.io-00d4aa?style=for-the-badge)](https://nvikou.github.io)
[![GitHub](https://img.shields.io/badge/💻_GitHub-@nvikou-181717?style=for-the-badge&logo=github)](https://github.com/nvikou)
[![Email](https://img.shields.io/badge/✉️_Email-nvikou@gmail.com-ea4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:nvikou@gmail.com?subject=Collaboration%20-%20Vikou%20Nelson)

<br>

⭐️ If you like my work, a **star** and a **follow** are always appreciated — thank you!

</div>
